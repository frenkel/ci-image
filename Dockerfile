...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y nodejs npm ruby-dev bundler build-essential zlib1g-dev libsqlite3-dev libpq-dev libmysqlclient-dev tzdata git ghostscript file imagemagick curl libcurl4-openssl-dev tnef chromium-browser && rm -rf /var/lib/apt/lists/*
