...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get dist-upgrade -y && DEBIAN_FRONTEND=noninteractive apt-get install -y nodejs nodejs-dev node-gyp npm ruby-dev bundler build-essential zlib1g-dev libsqlite3-dev libpq-dev libmysqlclient-dev tzdata git ghostscript file imagemagick curl libssl1.0-dev tnef chromium-browser xfonts-base xfonts-75dpi && rm -rf /var/lib/apt/lists/*
